...
import { sendCareTx } from "./care-tx.js";
...
async function boot() {
  ...
  // Onchain side-effect: trigger your contract on any CARE action (does not affect gameplay)
  els.feed.addEventListener("click", () => { sendCareTx("feed").catch(() => {}); });
  els.pet.addEventListener("click", () => { sendCareTx("pet").catch(() => {}); });
  els.clean.addEventListener("click", () => { sendCareTx("clean").catch(() => {}); });
